...
var (
	lottery_mysql      *gorm.DB
	lottery_mysql_once sync.Once
	dblog              ormlog.Interface

	lottery_redis *redis.Client
	lottery_once  sync.Once
)
...
func GetLotteryDBConnection() *gorm.DB {
	lottery_mysql_once.Do(func() {
		dbName := "lottery"
		...
		lottery_mysql = createMysqlDB(dbName, host, user, pass, port)
	})

	return lottery_mysql
}
...
func GetRedisClient() *redis.Client {
	lottery_once.Do(func() {
		viper := util.CreateConfig("redis")
		addr := viper.GetString("addr")
		pass := viper.GetString("pass")
		db := viper.GetInt("db")
		lottery_redis = createRedisClient(addr, pass, db)
	})
	return lottery_redis
}
